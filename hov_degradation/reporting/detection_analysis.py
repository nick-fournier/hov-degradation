...
class DetectionsPlot:
    ...
    def recast_wide(self):
        ...
        filters = {'Total': ['Data unavailable', 'Not misconfigured'],
                   ...
                   'Data unavailable': ['Not misconfigured',
                                        'Possible misconfiguration\n(Supervised only)',
                                        'Possible misconfiguration\n(Unsupervised only)']}
        ...
        for f in filters:
            ...
            f_counts = f_counts.append(
                pd.DataFrame({'ID': [x for x in df_wide.ID.unique() if x not in f_counts.ID.unique()], f: 0})
            )
            counts = pd.merge(counts, f_counts, on='ID')

        # Merge the counts and sort, also add version with meta data
        df_wide = df_wide.merge(right=counts, how='left', on='ID').sort_values('Total', ascending=False)
        df_wide_meta = df_wide.merge(right=self.meta, how='left', on='ID').sort_values('Total', ascending=False)
